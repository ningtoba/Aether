...
describe("RAGEngine", () => {
  describe("index", () => {
    ...
    it("should add chunk index metadata", async () => {
      const { store, engine } = createEngine({ maxChunkSize: 20, strategy: "fixed", overlap: 0, separator: "\n" });

      const ids = await engine.index("This is a longer document that should be split into multiple chunks.", {});
  ...
  describe("query", () => {
    ...
    it("should join multiple results with separator", async () => {
      const { store, engine } = createEngine({ maxChunkSize: 10, strategy: "fixed", overlap: 0, separator: "\n" });

      await engine.index(
        "First piece of context. Second piece. Third piece.",
        { test: true },
      );
