...
describe("MemoryStore", () => {
  ...
  describe("search", () => {
    beforeEach(async () => {
      await store.add({ type: "semantic", content: "The sky is blue and clear", metadata: { topic: "weather" } });
      await store.add({ type: "semantic", content: "Machine learning is fascinating", metadata: { topic: "ai" } });
      await store.add({ type: "task", content: "Buy groceries: milk and eggs", metadata: { priority: "high" } });
      await store.add({ type: "conversation", content: "User asked about blue sky", metadata: { topic: "weather" } });
    });

    it("should find entries by keyword matching", async () => {
      const results = await store.search({ query: "blue", type: undefined, limit: 10, threshold: 0 });
    ...
    it("should limit results", async () => {
      ...
      const results = await store.search({ query: "entry", type: undefined, limit: 3, threshold: 0 });
