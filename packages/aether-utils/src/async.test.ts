...
describe("withTimeout", () => {
  ...
  it("should reject when the promise takes too long", async () => {
    await expect(
      withTimeout(delay(200).then(() => "done"), 50),
    ).rejects.toThrow("Timed out after 50ms");
  });

  it("should use custom error message", async () => {
    await expect(
      withTimeout(delay(200).then(() => "done"), 50, "custom timeout"),
...
describe("retry", () => {
  it("should succeed on the first try", async () => {
    ...
    const result = await retry(fn, { maxAttempts: 3, baseDelay: 10, maxDelay: 100, backoff: "fixed" });
...
describe("raceWithTimeout", () => {
  ...
  it("should reject if no promise resolves in time", async () => {
    await expect(
      raceWithTimeout([
        delay(200).then(() => "too slow"),
      ], 50),
    ).rejects.toThrow("raceWithTimeout timed out");
  });
});
