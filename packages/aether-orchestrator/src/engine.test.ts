...
describe("LangGraphEngine", () => {
  ...
  beforeEach(() => { engine = new LangGraphEngine(); });
  ...
  describe("WorkflowBuilder", () => {
    it("should build fluent chains", () => {
      const w = new WorkflowBuilder("fluent", "2.0.0", "Fluent Test")
        .agentNode("start", "A").routerNode("router", "R", "D")
        .agentNode("a", "B").agentNode("b", "C").agentNode("end", "D")
        .connect("start", "router").connect("router", "a").connect("router", "b")
        .connect("a", "end").connect("b", "end")
        .withEntry("start").withTerminal("end")
        .withInitialStateField("input", "string", true).build();
  ...
  describe("GraphEditor", () => {
    it("should add and remove nodes", () => {
      const b = new WorkflowBuilder("d", "1.0.0").agentNode("a", "A").withEntry("a").withTerminal("a");
      const ed = new GraphEditor(b.build(true));
      expect(ed.edit({ type: "add-node", node: { id: "b", kind: "agent", agentName: "B" } }).success).toBe(true);
